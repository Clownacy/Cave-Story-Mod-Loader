...
void DrawHUD_Air_hijack(int x, int y)
{
	...
	DrawHUD_Air(native_width * (x / 320.), y);
}
...
void TextBoxDrawSpriteHijack(RECT* clip, int x, int y, RECT* src, int slot)
{
	...
	DrawSprite(clip, ((native_width - 244) / 2) + (x - 38), y, src, slot);
}

void ClearScreenHijack(RECT* rect, int flags)
{
	rect->left += (native_width - 320) / 2;
	rect->right += (native_width - 320) / 2;
...
//	WriteByte(0x41A350, 0xC3);
	...
	WriteLong(0x4219F9 + 6, ((native_width - 244) / 2) + 14);
	WriteLong(0x421A0D + 6, native_width - (((native_width - 244) / 2) + 14));
}
